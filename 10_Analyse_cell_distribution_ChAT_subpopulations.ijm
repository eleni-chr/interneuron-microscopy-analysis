/* STEP 10: ANALYSE CELL DISTRIBUTION FOR CHAT SUBPOPULATIONS SEPARATELY
...
This macro processes only the two ChAT⁺ subpopulations—interneurons and motor neurons—within each inner subfolder, using the central canal centroid already saved by STEP 6.  
For each inner subfolder it:
 1. Reads the central canal coordinates from “Central_canal_coordinates.csv”.  
 2. For each ChAT⁺ subpopulation (interneurons and motor neurons):
      • Loads its measurements CSV (e.g. “ch00_interneurons_measurements.csv”)  
      • Computes each cell’s distance & angle from the canal centroid  
      • Writes “ch00_<subpopuplation>_distances_angles.csv” with columns:  
...
for (i = 0; i < firstLevel.length; i++) {
    ...
    for (j = 0; j < innerList.length; j++) {
        ...
        dist1 = innerDir + "ch00_interneurons_distances_angles.csv";
        dist2 = innerDir + "ch00_motorNeurons_distances_angles.csv";
        if (File.exists(dist1) && File.exists(dist2)) {
        ...
        // 2) Process each ChAT⁺ subpopulation
        subs = newArray("interneurons", "motorNeurons");
        for (s = 0; s < lengthOf(subs); s++) {
            pop     = subs[s];
            measCSV = innerDir + "ch00_" + pop + "_measurements.csv";
            outCSV  = innerDir + "ch00_" + pop + "_distances_angles.csv";
